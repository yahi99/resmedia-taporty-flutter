...
class Database extends FirebaseDatabase
  ...
  Future<void> createUserGoogle(
    ...
    await fs.collection(collections.USERS).document(uid).setData({
  ...
  Future<CalendarModel> getDriverCalendarModel(
  ...
  Future<Coordinates> getRestaurantPosition(String restaurantId) async {
    final model = RestaurantModel.fromFirebase(
        await fs.collection(collections.RESTAURANTS).document(restaurantId).get());
  ...
  Future<void> pushReviewDriver(String driverId, int points, String strPoints,
      String uid, String orderId, String nominative) async {
    final model = UserModel.fromFirebase(
        await fs.collection('users').document(driverId).get());
    double average;
    int number;
    if (model.numberOfReviews != null) {
      average = (model.averageReviews * model.numberOfReviews + points) /
    ...
    await fs
        ...
        .document(driverId)
        .updateData({'numberOfReviews': number, 'averageReviews': average});
    await fs.collection('users').document(driverId).collection('reviews').add({
      'points': points,
      'strPoints': strPoints,
      'oid': orderId,
      'userId': uid,
  ...
  Future<void> setReviewed(String uid, String orderId) async {
    await fs
        .collection('users')
        .document(uid)
        .collection('user_orders')
  ...
  Future<void> updateTime(String day, String isLunch, String startTime,
      String endTime, String restaurantId) async {
    final rest = RestaurantModel.fromFirebase(
        await fs.collection('restaurants').document(restaurantId).get());
    if (isLunch == 'Pranzo') {
  ...
  Future<String> uploadImage(String path, File imageFile) async {
    StorageReference ref =
        fbStorage.ref().child(path).child(imageFile.path.split('/').last);
  ...
  Stream<List<ShiftModel>> getUsersTurn(String day) {
    final data =
        fs.collection(collections.DAYS).document(day).collection(collections.TIMES).snapshots();
  ...
  Future<String> getUserNominativeFromId(String uid) async {
    return UserModel.fromFirebase(
            await fs.collection(collections.USERS).document(uid).get())
        .nominative;
  }

  Future<RestaurantModel> getPos(String restId) async {
    return RestaurantModel.fromFirebase(
        await fs.collection(collections.RESTAURANTS).document(restId).get());
  }
