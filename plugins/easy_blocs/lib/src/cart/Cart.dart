...
class Cart {
  ...
  int getTotalItems(List<ProductCart> products) {
    return products.fold(0, (price, product) => price + product.countProducts);
  }
  ...
  bool increment(String id, String restaurantId, String userId, double price, String type) {
    ...
    return product == null ? onInsert(ProductCart(id: id, countProducts: 1, restaurantId: restaurantId, userId: userId, price: price, type: type)) : onIncrement(product.increment());
  }
  ...
  bool decrease(String id, String restaurantId, String userId) {
    ...
    return product.countProducts <= 0 ? onRemove(product) : onDecrease(product);
  }
...
class ProductCart {
  ...
  ProductCart({@required this.type, @required this.id, this.countProducts: 0, @required this.restaurantId, @required this.userId, @required this.price, this.delete = false})
      : assert(countProducts != null),
        assert(id != null);
  ...
  ProductCart deleteItem(bool delete) {
    return ProductCart(id: id, restaurantId: restaurantId, countProducts: countProducts, userId: userId, price: price, type: type, delete: delete);
  }

  ProductCart copyWith({int countProducts}) {
    return ProductCart(id: id, countProducts: countProducts, restaurantId: restaurantId, userId: userId, price: price, type: type, delete: delete);
  }
