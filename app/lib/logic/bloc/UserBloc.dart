...
class UserBloc with MixinFirebaseUserManager implements Bloc {
  ...
  UserBloc.instance() {
    _notificationController = PublishSubject(
        onListen: () {
    ...
    firebaseUserSub = _firebaseUserController.outFirebaseUser.listen((firebaseUser) async {
      ...
      userModelSub = _db.getUser(firebaseUser).listen((userModel) async {
        ...
        if (userModel.fcmToken != _user?.model?.fcmToken) {
          _fcmTokenSubscription?.cancel();
          _fcmTokenSubscription = await _db.autoRefreshFcmToken(userModel,
        ...
        if (!_firebaseUserController.registrationLevel.isCompleted)
          _firebaseUserController.registrationLevel.complete(RegistrationLevel.COMPLETE);
      });
    });
  ...
  void updateNominative(String nominative,String email)async{
    ...
    CloudFunctions.instance.getHttpsCallable(functionName: 'updateUser').call({'uid':restUser.uid,'nominative':nominative,'email':email,});
  }
