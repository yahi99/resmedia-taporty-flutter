...
class CacheManager extends BaseCacheManager {
  ...
  factory CacheManager.monthTwo({
    Duration maxAgeCacheObject = const Duration(days: 30),
    int maxNrOfCacheObjects = 128,
  }) {
    return CacheManager.cacheKey(
        "CacheManager#MontTwo",
        (cacheKey) => CacheManager(
              cacheKey: cacheKey,
              maxAgeCacheObject: maxAgeCacheObject,
              maxNrOfCacheObjects: maxNrOfCacheObjects,
  ...
  factory CacheManager.monthThree({
    Duration maxAgeCacheObject = const Duration(days: 30),
    int maxNrOfCacheObjects = 64,
  }) {
    return CacheManager.cacheKey(
        "CacheManager#MontThree",
        (cacheKey) => CacheManager(
              cacheKey: cacheKey,
              maxAgeCacheObject: maxAgeCacheObject,
              maxNrOfCacheObjects: maxNrOfCacheObjects,
  ...
  factory CacheManager.week({
    Duration maxAgeCacheObject = const Duration(days: 7),
  ...
  factory CacheManager.twoDay({
    Duration maxAgeCacheObject = const Duration(days: 2),
    int maxNrOfCacheObjects = 200,
  }) {
    return CacheManager.cacheKey(
        "CacheManager#TwoDay",
  ...
  factory CacheManager.cacheKey(String cacheKey, CacheManager builder(String cacheKey)) {
    if (!_instances.containsKey(cacheKey)) _instances[cacheKey] = builder(cacheKey);
