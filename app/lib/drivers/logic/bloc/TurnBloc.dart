...
class TurnBloc implements Bloc {
  ...
  Stream<Map<MonthCategory, List<TurnModel>>> get outCategorizedTurnsRest => outTurnsRest.map((models) {
        return categorized(MonthCategory.values, models, (model) => model.month);
      });
  ...
  Future setTurnStream(String uid) async {
    ...
    _turnControl = PublishController.catchStream(source: _db.getTurns(uid));
  ...
  Future setTurnRestStream() async {
    ...
    _turnControlRest = PublishController.catchStream(source: _db.getTurnsRest(restUser.model.restaurantId));
