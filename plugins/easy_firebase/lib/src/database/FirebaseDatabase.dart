...
class FirebaseNotificationFunctions {
  ...
  FirebaseNotificationFunctions.all(FirebaseNotification onNotification)
      : this(
...
class FirebaseDatabase {
  ...
  Future<void> setUser(FirebaseUser user, JsonRule json, {bool merge: false}) {
    return fs.document(users.id).setData(json.toJson(), merge: merge);
  }
  ...
  Future<StreamSubscription<String>> autoRefreshFcmToken(
    UserFirebaseModel model, {
    FirebaseNotificationFunctions notificationFunctions,
  }) async {
    if (Platform.isIOS) {
      fbMs.requestNotificationPermissions(IosNotificationSettings(sound: true, badge: true, alert: true));
  ...
  static List<V> fromQuerySnaps<V extends FirebaseModel>(
